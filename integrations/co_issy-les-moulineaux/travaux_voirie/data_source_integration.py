...
def compute_location_fields(df: pl.DataFrame) -> pl.DataFrame:
    ...
    gdf = gpd.GeoDataFrame(
        pdf, 
        geometry=gpd.GeoSeries.from_wkb(pdf["geolocalisation"]), 
        crs="EPSG:4326"
    )
...
def compute_regulation_fields(df: pl.DataFrame) -> pl.DataFrame:
    return df.with_columns([
        ...
        pl.lit(PostApiRegulationsAddBodyCategory.TEMPORARYREGULATION.value)
            .alias("regulation_category"),
        pl.lit(PostApiRegulationsAddBodySubject.ROADMAINTENANCE.value)
