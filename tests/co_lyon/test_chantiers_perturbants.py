...
def test_reads_hours_out_of_free_text(description, expected):
    """The parser alone: what it can read, whatever else the text says."""
    assert parse_time_slots(description) == expected
...
@pytest.mark.parametrize(
    ...
    [
        ("7h-17h", [("07:00", "17:00")]),
        ("7h30-12h et 13h-16h30", [("07:30", "12:00"), ("13:00", "16:30")]),
        (
            "11h-15h / 19h-00h / 00h-1h",
            [("11:00", "15:00"), ("19:00", "00:00"), ("00:00", "01:00")],
        ),
        ("entre 7h30 et 18h30", [("07:30", "18:30")]),
        ("De 21:00 à 05:00,", [("21:00", "05:00")]),
        ("Sauf riverains", []),
        ("de 8h à 16h30 sauf riverains", [("08:00", "16:30")]),
        (None, []),
    ],
)
def test_keeps_rows_whose_description_is_read_in_full(description, expected):
    """Hours, a resident exemption, and connectors between them: nothing is lost (R-78)."""
    result = timed(
...
@pytest.mark.parametrize(
    "description,reason",
    [
        # A direction, which a zone cannot carry.
        ("Sens Nord/Sud", "says more than hours"),
        ("De 7h à 17h, sens Sud/Nord", "says more than hours"),
        ("Dans le sens Est-Ouest", "says more than hours"),
        # A nuance DiaLog cannot express.
        ("Réouverture de la route les week ends", "says more than hours"),
        ("Circulation interdite pour des periodes de 15 minutes", "says more than hours"),
        ("Déviations sur place pour tous transports", "says more than hours"),
        # An exemption beyond residents.
        ("Sauf riverains et livraisons", "says more than hours"),
        # A clock reading outside any pair: the hours were not read in full.
        ("de 7h à 17h, réouverture à 18h", "not read in full"),
        ("à partir de 8h", "not read in full"),
    ],
)
def test_drops_rows_whose_description_says_what_we_cannot_publish(description, reason, warnings):
    """Half-reading the description would publish a stronger restriction than stated."""
    result = timed(
        row(typeperturbation="Circulation interdite", descripchantierinternet=description)
    )
    assert result.height == 0
...
def test_residual_threshold_sits_between_connectors_and_words():
    """`entre … et` (7 characters) passes, `Sens Nord/Sud` (12) does not."""
    assert read_description("entre 7h30 et 18h30").residual == "entreet"
    assert read_description("entre 7h30 et 18h30").fully_read
    assert read_description("Sens Nord/Sud").residual == "SensNord/Sud"
    assert not read_description("Sens Nord/Sud").fully_read
...
@pytest.mark.parametrize(
    ...
    [
        ("Circulation interdite de jour", "de 21h à 5h"),
        ("Circulation interdite de nuit", "de 8h à 17h"),
    ],
)
def test_drops_rows_whose_type_contradicts_their_hours(perturbation, description, warnings):
    result = timed(row(typeperturbation=perturbation, descripchantierinternet=description))
    assert result.height == 0
    assert any("contradicts" in message for message in warnings)


def test_a_night_window_on_a_plain_closure_is_not_a_contradiction():
    """`Circulation interdite` + `21h-6h` is a night closure, not an inconsistency."""
    result = timed(row(typeperturbation="Circulation interdite", descripchantierinternet="21h-6h"))
    assert result.height == 1
...
def test_location_is_a_zone_holding_a_single_polygon_and_builds_a_label():
    result = compute_location_fields(frame())
    assert result.get_column("location_road_type").to_list() == ["zone"]
    geometry = json.loads(result.get_column("location_geometry")[0])
    assert geometry["type"] == "Polygon"
    assert geometry["coordinates"] == json.loads(GEOMETRY)["coordinates"][0]
...
def test_location_splits_a_multipart_footprint_into_one_zone_per_part():
    two_parts = json.dumps(
        {
            "type": "MultiPolygon",
            "coordinates": [
                [[[4.83, 45.76], [4.84, 45.76], [4.83, 45.77], [4.83, 45.76]]],
                [[[4.85, 45.76], [4.86, 45.76], [4.85, 45.77], [4.85, 45.76]]],
            ],
        }
    )
    result = compute_location_fields(frame(row(geometry=two_parts)))
    assert result.height == 2
    assert result.get_column("gid").to_list() == [415731, 415731]
    assert [json.loads(g)["type"] for g in result.get_column("location_geometry")] == [
        "Polygon",
        "Polygon",
    ]
...
def test_location_drops_footprints_that_are_not_polygons():
    line = json.dumps({"type": "LineString", "coordinates": [[4.83, 45.76], [4.84, 45.76]]})
    assert compute_location_fields(frame(row(geometry=line))).height == 0
...
def test_title_strips_the_producer_trailing_spaces():
    result = compute_regulation_fields(
        frame(row(nomchantier="Travaux de branchement ", nom=" Rue X"))
    )
    assert result.get_column("regulation_title")[0] == "Travaux de branchement – Rue X"
...
def test_all_vehicles_by_default():
    ...
    assert result.get_column("vehicle_exempted_types").to_list() == [None]


@pytest.mark.parametrize(
    "description", ["Sauf riverain", "Sauf riverains", "7h-17h sauf riverains"]
)
def test_residents_are_exempted_when_the_description_says_so(description):
    """`Sauf riverains` is the one exemption the layer spells out (R-36)."""
    result = compute_vehicle_fields(frame(row(descripchantierinternet=description)))
    assert result.get_column("vehicle_all_vehicles").to_list() == [True]
    assert result.get_column("vehicle_exempted_types").to_list() == [["localResident"]]
